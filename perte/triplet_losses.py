...
class TripletLoss(nn.Module):
    ...
    Triplet loss
    """
    def __init__(self, alpha=0.5, reduction="mean", device=torch.device("cpu")):
...
class OnlineTripletLoss(nn.Module):
    ...
    def __init__(self, triplet_selector: TripletSelector, margin: float=0.5, reduction="mean", device=torch.device("cpu")):
...
class BatchHardTripletSelector(TripletSelector):
    ...
    def get_triplets(self,embeddings,labels, inputs):
        ...
        for anchor,label in enumerate(labels):
            ## the number of triplet pairs belongs to the batch of labels
